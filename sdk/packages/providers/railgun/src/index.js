...
exports.RailgunProvider = void 0;
const core_1 = require("@zksdk/core");
// Railgun SDK imports
const engine_1 = require("@railgun-community/engine");
class RailgunProvider extends core_1.BasePrivacyProvider {
    constructor(config = {}) {
        super(config);
        this.name = 'Railgun';
    ...
    async initialize(config) {
        ...
        try {
            ...
            this.railgunEngine = new engine_1.RailgunEngine(config.engineDbPath, config.rpcEndpoints, undefined, // proxy settings
            false, // shouldDebug
            undefined, // wasmPath
            ...
            if (config.walletMnemonic) {
                this.railgunWallet = await engine_1.RailgunWallet.fromMnemonic(this.railgunEngine, config.walletMnemonic, undefined, // derivationIndex
                false // isViewOnly
                );
            }
    ...
    async transfer(params) {
        ...
        try {
            ...
            const proof = await this.railgunEngine.generateProof(this.railgunWallet.id, railgunNetwork, recipients, [], gasDetails, [], engine_1.ProofType.BALANCE_CHECK);
            // Submit transaction
            console.log('Submitting transaction...');
            const txHash = await this.railgunEngine.submitTransaction(railgunNetwork, proof);
            return {
     ...
     */
    getRailgunNetwork(network) {
        const networkMap = {
            'ethereum': engine_1.NetworkName.Ethereum,
            'polygon': engine_1.NetworkName.Polygon,
            'arbitrum': engine_1.NetworkName.Arbitrum
        };
    ...
    getExplorerUrl(network, txHash) {
        const explorerMap = {
            [engine_1.NetworkName.Ethereum]: 'https://etherscan.io/tx/',
            [engine_1.NetworkName.Polygon]: 'https://polygonscan.com/tx/',
            [engine_1.NetworkName.Arbitrum]: 'https://arbiscan.io/tx/'
        };
...
exports.RailgunProvider = RailgunProvider;
